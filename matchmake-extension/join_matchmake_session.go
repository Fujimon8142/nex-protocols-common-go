...
func (commonProtocol *CommonProtocol) joinMatchmakeSession(err error, packet nex.PacketInterface, callID uint32, gid types.UInt32, strMessage types.String) (*nex.RMCMessage, *nex.Error) {
	...
	// Call the secure join function which includes blocklist checks
	_, nexError = database.JoinMatchmakeSessionBlockList(
		commonProtocol.manager,
		joinedMatchmakeSession,
		connection,
		1, // participationCount
		string(strMessage),
		false, // dontCareMyBlockList
	)
